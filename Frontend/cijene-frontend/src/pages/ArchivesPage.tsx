      ...
      <div className="bg-white p-6 rounded-lg shadow-md mb-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div className="text-center p-4 bg-blue-50 rounded-lg">
            ...
            <p className="text-2xl font-bold text-blue-600 mt-2">{archives.length}</p>
          </div>
          ...
          {filteredArchives.length === 0 ? (
            ...
            paginatedArchives.map((archive: Archive) => {
              ...
              return (
                <div
                  key={archive.date}
                  className={`p-6 transition-colors flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 ${isSelected ? 'bg-blue-50' : 'hover:bg-gray-50'
                    }`}
                >
                  <div className="flex items-start gap-4 flex-1">
                    ...
                    <div>
                      <h3 className="text-lg font-medium text-gray-900">
                        {t('archives.item.title').replace('{date}', new Date(archive.date).toLocaleDateString(undefined, { dateStyle: 'full' }))}
                      </h3>
        ...
        {totalPages > 1 && (
          <div className="p-4 border-t border-gray-200 flex items-center justify-center gap-4">
            ...
            <div className="flex items-center gap-2">
              <span className="text-gray-600">{t('common.page')}</span>
              <span className="font-semibold text-blue-600">{currentPage}</span>
              <span className="text-gray-600">{t('common.of')}</span>
