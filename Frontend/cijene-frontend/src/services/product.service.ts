...
import { apiClient, RequestValidator, ApiError } from './api-client';
import { BaseService } from './base.service';
import { URLParamsBuilder } from '../utils/urlParams';
import { ENDPOINTS, PAGINATION, API_CONFIG } from '../constants';
...
export class ProductService extends BaseService {
  ...
  async searchProducts(
    ...
    try {
      ...
      // If the API is not accessible and mock data is enabled, use mock data
      if (API_CONFIG.ENABLE_MOCK_DATA && error instanceof ApiError &&
        (error.code === 'DNS_ERROR' || error.code === 'NETWORK_ERROR' || error.code === 'CONNECTION_REFUSED')) {
        console.warn('API not accessible, using mock data:', error.message);
        return await mockDataService.getMockProducts(params);
      }
  ...
  async getProductPrices(
    ...
    if (params.chains) searchParams.append('chains', params.chains);
