...
import { ENDPOINTS, PAGINATION, LOCATION, API_CONFIG } from '../constants';
  ...
  async searchStores(
    ...
    try {
      ...
      if (API_CONFIG.ENABLE_MOCK_DATA && error instanceof ApiError &&
        ...
        console.warn('API not accessible, using mock data for stores:', error.message);
        return await mockDataService.getMockStores(params);
      }
