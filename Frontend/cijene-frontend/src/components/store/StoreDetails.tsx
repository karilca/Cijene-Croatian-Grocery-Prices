...
export const StoreDetails: React.FC<StoreDetailsProps> = ({
  ...
  const { addStore, removeStore, isStoreFavorite } = useFavoriteActions();
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex gap-2">
            ...
            >
              ...
              {isFavorite ? t('storeDetails.favorited') : t('storeDetails.addToFavorites')}
            </button>
            ...
            <div className="space-y-4">
              ...
              {store.address && (
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    <div className="text-sm font-medium text-gray-700">{t('storeDetails.address')}</div>
                    <div className="text-gray-600">{store.address}</div>
                    {store.city && (
                      <div className="text-gray-600">{store.city}</div>
                    )}
                  </div>
                </div>
              )}
              ...
              {distance !== null && (
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    <div className="text-sm font-medium text-gray-700">{t('storeDetails.distance')}</div>
                    <div className="text-gray-600">{t('storeDetails.fromYourLocation').replace('{distance}', formatDistance(distance))}</div>
                  </div>
                </div>
              )}
              ...
              {store.latitude && store.longitude && (
                <div className="flex items-start gap-3">
                  <MapPin className="h-5 w-5 text-gray-400 mt-0.5 flex-shrink-0" />
                  <div>
                    <div className="text-sm font-medium text-gray-700">{t('storeDetails.coordinates')}</div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            ...
            {distance !== null && (
              <div className="bg-blue-50 p-4 rounded-lg text-center">
                <div className="text-2xl font-bold text-blue-900">{formatDistance(distance)}</div>
                <div className="text-sm text-blue-600">{t('storeDetails.distance')}</div>
              </div>
            )}
