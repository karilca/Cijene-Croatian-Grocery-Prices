...
import type { StoreSearchRequest } from '../../types';
...
export const StoreSearch: React.FC<StoreSearchProps> = ({ onSearch, className = '' }) => {
  ...
  const handleSearch = useCallback((searchQuery?: string, searchCity?: string) => {
    const finalQuery = searchQuery || query;
    const finalCity = searchCity || city;

    if (!finalQuery.trim() && !finalCity.trim() && !position && !defaultLocation.latitude) return;
  ...
  }, [getCurrentPosition, geoError, clearGeoError]);
      ...
      <div className="flex flex-wrap gap-4 items-end">
        ...
        <div>
          ...
          <button
            ...
            disabled={geoLoading}
            className="flex items-center gap-2 px-4 py-3 border border-gray-300 rounded-lg hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent disabled:opacity-50 disabled:cursor-not-allowed"
          >
            {geoLoading ? (
              <LoadingSpinner size="sm" />
            ) : (
        ...
        <div>
          <button
            ...
            className="flex items-center gap-2 px-4 py-3 border border-gray-300 rounded-lg hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent"
          >
